...
async fn websocket_ready(base_url: &str, token: &str) {
    ...
    let mut auth = HeaderValue::from_str(&format!("codeg-events, codeg-token.{}", URL_SAFE_NO_PAD.encode(token))).unwrap();
    auth.set_sensitive(true);
    request.headers_mut().insert("sec-websocket-protocol", auth);
    let (mut socket, _) = tokio::time::timeout(Duration::from_secs(10), tokio_tungstenite::connect_async(request)).await.unwrap().unwrap();
...
async fn isolated_sshd_install_reuse_tunnel_and_reconnect() {
    ...
    let config = RemoteWorkspaceSshConfig { host, ..Default::default() };
    let db = crate::db::test_helpers::fresh_in_memory_db().await;
    let profile = profiles::create(&db.conn, "CI SSH", "", "", &[], Some(&config)).await.unwrap();
    let manager = SshManager::new();
    manager.register_window(profile.id, "first-window");
    let (first, concurrent) = tokio::join!(manager.resolve_connection(&db.conn, profile.id), manager.resolve_connection(&db.conn, profile.id));
    let first = first.unwrap();
    let concurrent = concurrent.unwrap();
    assert_eq!(first.base_url, concurrent.base_url, "concurrent calls must share one tunnel");
    ...
    assert_eq!(client.post(&url).json(&serde_json::json!({})).send().await.unwrap().status(), reqwest::StatusCode::UNAUTHORIZED);
    let health: serde_json::Value = client.post(&url).bearer_auth(&first.token).json(&serde_json::json!({})).send().await.unwrap().json().await.unwrap();
    assert_eq!(health["status"], "ok");
    assert_eq!(health["version"], env!("CARGO_PKG_VERSION"));
    websocket_ready(&first.base_url, &first.token).await;

    let reused = run_bootstrap(&config).await.unwrap();
    assert!(reused.reused, "a second SSH exec must adopt the background server");
    ...
    let second = manager.resolve_connection(&db.conn, profile.id).await.unwrap();
    assert!(first.token == second.token, "reconnection must retain the remote instance");
    websocket_ready(&second.base_url, &second.token).await;

    manager.window_closed(profile.id, "first-window");
    assert!(manager.resolve_connection(&db.conn, profile.id).await.is_err(), "late requests from a closed window must not recreate a tunnel");
    let port = reqwest::Url::parse(&second.base_url).unwrap().port().unwrap();
    tokio::time::timeout(Duration::from_secs(5), async {
        while tokio::net::TcpStream::connect(("127.0.0.1", port)).await.is_ok() {
            tokio::time::sleep(Duration::from_millis(50)).await;
        }
    }).await.unwrap();
    assert!(run_bootstrap(&config).await.unwrap().reused, "closing local tunnels must leave remote jobs running");

    manager.register_window(profile.id, "reopened-window");
    let reopened = manager.resolve_connection(&db.conn, profile.id).await.unwrap();
    websocket_ready(&reopened.base_url, &reopened.token).await;
    profiles::delete(&db.conn, profile.id).await.unwrap();
    manager.shutdown(profile.id).await;
    assert!(manager.resolve_connection(&db.conn, profile.id).await.is_err());
