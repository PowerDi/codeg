...
impl PromptHandler for FixedPrompter {
    async fn request(&self, owner: &str, _host: &str, kind: PromptKind, _prompt: &str) -> Option<Zeroizing<String>> {
        assert_eq!(owner, "test-window");
        self.calls.fetch_add(1, Ordering::Relaxed);
        if self.decline { return None; }
        Some(Zeroizing::new(if kind == PromptKind::HostKey { "yes" } else { " test-secret " }.into()))
    }
}

fn context(decline: bool) -> (ServerContext, Arc<FixedPrompter>) {
    let prompter = Arc::new(FixedPrompter { calls: AtomicUsize::new(0), decline });
    (ServerContext {
        token: "capability".into(), host: "server".into(), owner_window: "test-window".into(),
        prompter: prompter.clone(), credentials: Arc::new(CredentialCache::default()),
        failure: Arc::new(Mutex::new(None)), requests: AtomicUsize::new(0),
    }, prompter)
}
...
fn classifies_only_supported_openssh_prompts() {
    ...
    assert_eq!(prompt_kind("Enter passphrase for key 'C:\\Users\\Alice\\.ssh\\id_ed25519': "), Some(PromptKind::Passphrase));
    assert_eq!(prompt_kind(HOST_PROMPT), Some(PromptKind::HostKey));
    assert_eq!(prompt_kind("Enter OTP:"), None);
    assert_eq!(prompt_kind("Are you sure you want to continue connecting?"), None);
}

#[tokio::test]
async fn caches_exact_prompts_but_never_host_key_approvals() {
    let (context, prompter) = context(false);
    assert_eq!(context.answer(PASSWORD_PROMPT).await.unwrap().as_str(), " test-secret ");
    ...
    assert!(!context.credentials.answers.lock().unwrap().contains_key(HOST_PROMPT));
    assert!(!context.credentials.redact("echo  test-secret ").contains("test-secret"));
...
fn pending(broker: &PromptBroker, kind: PromptKind, expires_at: i64) -> oneshot::Receiver<Option<Zeroizing<String>>> {
    let (response, receiver) = oneshot::channel();
    broker.pending.lock().unwrap().insert("request".into(), PendingPrompt {
        payload: PromptPayload {
            request_id: "request".into(), owner_window: "owner".into(), host: "server".into(),
            kind, prompt: PASSWORD_PROMPT.into(), expires_at,
        }, response,
    });
    receiver
}
...
async fn only_the_owner_may_inspect_or_answer_a_request() {
    ...
    assert!(broker.answer("other", "request", Some("wrong-window".into())).is_err());
    assert_eq!(broker.list("owner").len(), 1);
    broker.answer("owner", "request", Some(" secret ".into())).unwrap();
...
async fn host_approval_is_explicit_and_expired_answers_fail_closed() {
    ...
    assert!(broker.answer("owner", "request", Some("true".into())).is_err());
    ...
    broker.answer("owner", "request", Some("late-secret".into())).unwrap();
    assert!(receiver.await.unwrap().is_none());
}
...
async fn helper_socket_requires_the_ephemeral_capability() {
    ...
    let server = AskpassServer::start(PathBuf::from("codeg.exe"), "server".into(), "test-window".into(),
        prompter.clone(), Arc::new(CredentialCache::default())).await.unwrap();
    for (token, expected) in [("wrong-token".to_string(), false), (server.token.clone(), true)] {
        let mut socket = TcpStream::connect(&server.address).await.unwrap();
        let mut request = serde_json::to_vec(&AskpassRequest { token, prompt: PASSWORD_PROMPT.into() }).unwrap();
        request.push(b'\n');
        socket.write_all(&request).await.unwrap();
        let mut response = Vec::new();
        tokio::time::timeout(Duration::from_secs(2), socket.read_to_end(&mut response)).await.unwrap().unwrap();
        if expected {
            let reply: super::super::askpass_client::AskpassReply = serde_json::from_slice(&response).unwrap();
            assert_eq!(reply.answer.as_deref(), Some(" test-secret "));
        } else { assert!(response.is_empty()); }
    }
    assert_eq!(prompter.calls.load(Ordering::Relaxed), 1);
    let mut command = crate::ssh::command::ssh_command_with_askpass(
        &crate::models::RemoteWorkspaceSshConfig { host: "server".into(), ..Default::default() },
        crate::ssh::command::SshInvocation::Exec, Some("true"), Some(&server));
    // No actual password in process inspection, shell commands or environment.
    for argument in command.as_std().get_args() { assert!(!argument.to_string_lossy().contains("test-secret")); }
    for (_, value) in command.as_std().get_envs() {
        assert!(!value.unwrap_or_default().to_string_lossy().contains("test-secret"));
    }
