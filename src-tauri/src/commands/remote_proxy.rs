...
// `app`/`proxy`/`connection_id`/`database`/`entry`/`shutdown_rx`/
    ...
    'reconnect: loop {
        ...
        let connect_result = tokio::select! {
            ...
            res = async {
                ...
                let ws_url = http_url_to_ws_url(&conn.base_url, WS_EVENTS_PATH);
                let headers = conn.headers.to_header_map();
                connect_with_subprotocol_auth(&ws_url, WS_EVENT_PROTOCOL, &conn.token, &headers)
